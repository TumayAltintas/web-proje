...
export default {
  ...
  async mounted() {
    ...
    this.querySnapshots = await getDocs(collection(db, 'camasirmakinesi'));
    if (this.getcategorysleng > 0) {
      this.querySnapshots = await getDocs(query(collection(db, "camasirmakinesi"), where(this.bywhere, "in", this.incategorys)));
    }
    let productlist = []
