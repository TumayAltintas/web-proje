...
export default {
  ...
  async mounted() {
    ...
    this.querySnapshots = await getDocs(collection(db, 'sporayakkabı'));
    if (this.getcategorysleng > 0) {
      this.querySnapshots = await getDocs(query(collection(db, "sporayakkabı"), where(this.bywhere, "in", this.incategorys)));
    }
    let productlist = []
